...
dotenv.config()
route.get('/', (req, res) => {
    res.redirect('https://sfg-solver.herokuapp.com/login')
})
...
function sendmail(tomailid, hash, fp) {
    ...
    if (fp == 1) {
        mailDetails = {
            ...
            text: 'Reset your password by clicking on the link (link is valid upto five minutes only) ' + 'https://sfg-solver.herokuapp.com/forgot?id=' + hash + '&tm=' + tm + '&mail=' + tomailid,
        };
    } else if (fp == 0) {
        mailDetails = {
            from: process.env.mail,
            ...
            text: 'Verify your account by clicking on the link ' + 'https://sfg-solver.herokuapp.com/activate?id=' + hash + '&mail=' + tomailid + '&tm=' + tm + ' .' + 'This Link will expire in 10 minutes',
        };
    }
